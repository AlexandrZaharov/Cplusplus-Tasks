...
int Forest::GetBurnNeighbour(int y, int x)
{
    ...
    for (int j = y - 1; j <= y + 1; j++)
    {
        for (int i = x - 1; i <= x + 1; i++)
        {
            ...
            {
                if (data_[j][i].current_status_ == STATUS_BURN)
                {
...
void Forest::Update()
{
    for (int y = 0; y < data_.size(); y++)
    {
        for (int x = 0; x < data_[0].size(); x++)
        {
            ...
            if (data_[y][x].current_status_ == STATUS_GROW)
            {
            ...
            if (data_[y][x].current_status_ == STATUS_BURN)
            {
    ...
    for (int y = 0; y < data_.size(); y++)
    {
        for (int x = 0; x < data_[0].size(); x++)
        {
            if (data_[y][x].current_status_ == STATUS_DEAD)
            {
                data_[y][x].current_status_ = STATUS_GROW;
                data_[y][x].strength_ = 5;
                data_[y][x].health_ = 10;
            }
            if (data_[y][x].strength_ == 0 && data_[y][x].health_ != 0)
            {
                data_[y][x].current_status_ = STATUS_BURN;
            }
            if (data_[y][x].health_ == 0)
            {
                data_[y][x].current_status_ = STATUS_DEAD;
            }
        }
    }
}
...
std::ostream& operator<<(std::ostream& output_stream, const Forest& matrix)
{
    for (size_t y = 0; y < matrix.GetHeight(); y++)
    {
        ...
        {
            if (matrix.GetCurrentStatus(y, x) == STATUS_GROW)
            {
                output_stream << '|' << " ";
            }
            if (matrix.GetCurrentStatus(y, x) == STATUS_BURN)
            {
                output_stream << '*' << " ";
            }
            if (matrix.GetCurrentStatus(y, x) == STATUS_DEAD)
            {
                output_stream << ' ' << " ";
            }
        }
