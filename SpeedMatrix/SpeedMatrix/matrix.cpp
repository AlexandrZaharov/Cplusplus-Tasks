...
{ LOG_DURATION("Operator* time is")

    Matrix new_matrix(data_.size(), rhs.GetWidth());
    const Matrix rhs_transpose = rhs.Transpose();
    int element = 0;
    for (size_t i = 0; i < data_.size(); i++)
    {
        for (size_t j = 0; j < rhs_transpose.GetHeight(); j++)
        {
            for (size_t k = 0; k < data_[0].size(); k++)
            {
                element += data_[i][k] * rhs_transpose.GetValue(j, k);
            }
